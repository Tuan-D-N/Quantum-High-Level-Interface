...
static CSR make_random_csr(int d, double density, std::mt19937_64 &rng)
{
    ...
    for (int r = 0; r < d; ++r)
    {
        ...
        if (want_this_row == 0 && nnz_so_far < nnz_target)
            want_this_row = 1;
                         ...
                         std::mt19937_64 &rng)
{
    ...
    // Pre-build const spans for sparse call
    std::span<const int> rowPtrSpan(csr.rowPtr.data(), csr.rowPtr.size());
    std::span<const int> colIndSpan(csr.colInd.data(), csr.colInd.size());
    std::span<const cplx> valsSpan(csr.values.data(), csr.values.size());
    ...
    // ----- Warm-up both paths once -----
    {
        ...
        // Sparse
        auto st = qs.applySparseGateToSV(rowPtrSpan, colIndSpan, valsSpan,
                                         targets, controls);
    }
...
int main_runner()
{
    ...
    const int repeats = 7;                 // per (nQubits, kTargets, density)
    const uint64_t seed = 0xC0FFEE1234ULL; // deterministic

    // CSV setup
    std::ofstream csv("sparse_bench_results.csv");
    if (!csv) {
        std::cerr << "Failed to open bench_results.csv for writing.\n";
        return 1;
    }
    ...
    // Track turning points per (nQubits,k): first density where sparse beats dense (speedup>1)
    struct Turning { int nQubits; int k; double density; double speedup; };
    std::vector<Turning> turns;

    for (int nQubits_system = 6; nQubits_system <= 20; ++nQubits_system)
    {
        // ------- State init for this nQubits -------
        QState qs(nQubits_system);
        qs.zero_state();
        qs.prefetchToDevice(); // ensure resident and fair
        randomize_state(qs, rng);

        const int kTargetsMax = std::min(nQubits_system, 10);
        ...
        for (int kTargets = 1; kTargets <= kTargetsMax; ++kTargets)
        {
            ...
            for (double density : densities)
            {
                ...
                // Also mirror to stdout as before
                ...
                {
                    turns.push_back(Turning{nQubits_system, kTargets, density, row.speedup_sparse_vs_dense});
                    found_turn = true;
                }
            }

            if (!found_turn) {
                turns.push_back(Turning{nQubits_system, kTargets, -1.0, 0.0}); // no win found
            }
        }

        qs.prefetchToCPU(); // optional between sizes
    }
